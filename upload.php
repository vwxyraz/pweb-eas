...
/*if(isset($_POST["submit"])) {
  ...
  if($check !== false) {
    echo "<script type='text/javascript'>alert('File is an image - ')</script>" . $check["mime"] . ".";
    $uploadOk = 1;
  } else {
...
// if everything is ok, try to upload file
  ...
  if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
    echo "<script type='text/javascript'>alert('The file ')</script>". htmlspecialchars( basename( $_FILES["fileToUpload"]["name"])). " has been uploaded.";
  } else {
